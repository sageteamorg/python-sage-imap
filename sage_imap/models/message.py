...
class MessageSet:
    ...
    **Batch operations:**
    ...
    def from_uids(cls, uids: List[int], mailbox: Optional[str] = None) -> "MessageSet":
        ...
        validated_uids = sorted(
            set(uid for uid in uids if isinstance(uid, int) and uid > 0)
        )

        if not validated_uids:
            raise ValueError("No valid UIDs provided")

        return cls(msg_ids=validated_uids, is_uid=True, mailbox=mailbox)

    @classmethod
    def from_sequence_numbers(
        cls, seq_nums: List[int], mailbox: Optional[str] = None
    ) -> "MessageSet":
        """
        ...
        **⚠️ WARNING: Sequence numbers are unreliable for production use!**
        ...
        validated_seq_nums = sorted(
            set(seq for seq in seq_nums if isinstance(seq, int) and seq > 0)
        )
        ...
        return cls(msg_ids=validated_seq_nums, is_uid=False, mailbox=mailbox)
    ...
    def from_email_messages(cls, messages: List["EmailMessage"]) -> "MessageSet":
        ...
        seq_nums = [
            msg.sequence_number for msg in messages if msg.sequence_number is not None
        ]
        ...
        raise ValueError(
            "No valid UIDs or sequence numbers found in EmailMessage objects"
        )
    ...
    def from_range(
        ...
        return cls(msg_ids=range_str, is_uid=is_uid, mailbox=mailbox)

    @classmethod
    def all_messages(
        cls, is_uid: bool = True, mailbox: Optional[str] = None
    ) -> "MessageSet":
        """
        ...
        if isinstance(self.msg_ids, list):
            ...
            for msg_id in self.msg_ids:
                if isinstance(msg_id, (int, str)):
                    ...
                    raise TypeError(
                        f"Message ID must be int or string, got {type(msg_id)}"
                    )
        ...
        if ":" in component:
            ...
            if end == "*":
                ...
                if start_val > end_val:
                    raise ValueError(
                        f"Invalid range: start ({start_val}) > end ({end_val})"
                    )
            else:
    ...
    def subtract(self, other: "MessageSet") -> "MessageSet":
        ...
        return MessageSet(
            msg_ids=list(remaining_ids), is_uid=self.is_uid, mailbox=self.mailbox
        )
    ...
    def split_by_size(self, max_size: int) -> List["MessageSet"]:
        ...
        if self.parsed_ids and not self.id_ranges:
            ...
            for i in range(0, len(ids), max_size):
                ...
                chunks.append(
                    MessageSet(
                        msg_ids=chunk_ids, is_uid=self.is_uid, mailbox=self.mailbox
                    )
                )
...
def create_sequence_set(
    seq_nums: List[int], mailbox: Optional[str] = None
) -> MessageSet:
...
def merge_message_sets(sets: List[MessageSet]) -> MessageSet:
    ...
    for msg_set in sets:
        if msg_set.mailbox and msg_set.mailbox != mailbox:
            logger.warning("Merging message sets from different mailboxes")

        combined_ids.append(msg_set.msg_ids)

    return MessageSet(
        msg_ids=",".join(combined_ids), is_uid=first_set.is_uid, mailbox=mailbox
    )
