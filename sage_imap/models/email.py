...
class EmailMessage:
    ...
    def parse_eml_content(self) -> None:
        ...
        try:
            ...
            self.to_address = self._parse_email_addresses(email_message.get_all("to", []))
            self.cc_address = self._parse_email_addresses(email_message.get_all("cc", []))
            self.bcc_address = self._parse_email_addresses(email_message.get_all("bcc", []))
                ...
                if "attachment" in content_disposition or part.get_filename():
                    try:
                        ...
                        if not filename:
                            ...
                            ext = self._get_extension_from_content_type(part.get_content_type())
    ...
    def is_multipart(self) -> bool:
        ...
        return bool(self.attachments) or (bool(self.plain_body) and bool(self.html_body))
    ...
    def get_attachments_by_type(self, content_type: str) -> List[Attachment]:
        ...
        return [att for att in self.attachments if att.content_type.startswith(content_type)]
...
class EmailIterator:
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[EmailMessage, "EmailIterator"]:
        if isinstance(index, int):
            ...
            if self._filtered_indices is not None:
                filtered_emails = [self._email_list[i] for i in self._filtered_indices[index]]
            else:
    ...
    def __reversed__(self) -> "EmailIterator":
        ...
        if self._filtered_indices is not None:
            reversed_emails = [self._email_list[i] for i in reversed(self._filtered_indices)]
        else:
    ...
    def count(self, condition: Callable[[EmailMessage], bool]) -> int:
        ...
        if self._filtered_indices is not None:
            return sum(1 for i in self._filtered_indices if condition(self._email_list[i]))
    ...
    def filter(self, criteria: Callable[[EmailMessage], bool]) -> "EmailIterator":
        ...
        if self._filtered_indices is not None:
            filtered_emails = [self._email_list[i] for i in self._filtered_indices if criteria(self._email_list[i])]
        else:
            filtered_emails = [email for email in self._email_list if criteria(email)]
        return EmailIterator(filtered_emails)

    def filter_by_header(self, key: str, value: Optional[str] = None) -> "EmailIterator":
    ...
    def filter_by_subject_regex(self, pattern: str, flags: int = 0) -> "EmailIterator":
        ...
        return self.filter(lambda email: compiled_pattern.search(email.subject) is not None)

    def filter_by_sender(self, sender: str, exact_match: bool = False) -> "EmailIterator":
        """Filter by sender email address."""
        if exact_match:
            return self.filter(lambda email: email.from_address and str(email.from_address) == sender)
        else:
            sender_lower = sender.lower()
            return self.filter(lambda email: email.from_address and sender_lower in str(email.from_address).lower())
    ...
    def filter_by_recipient(self, recipient: str, exact_match: bool = False) -> "EmailIterator":
        ...
        if exact_match:
            return self.filter(lambda email: any(str(addr) == recipient for addr in email.all_recipients))
        else:
            recipient_lower = recipient.lower()
            return self.filter(lambda email: any(recipient_lower in str(addr).lower() for addr in email.all_recipients))

    def filter_by_date_range(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> "EmailIterator":
    ...
    def filter_by_size_range(self, min_size: Optional[int] = None, max_size: Optional[int] = None) -> "EmailIterator":
    ...
    def filter_by_attachment_count(self, min_count: int = 0, max_count: Optional[int] = None) -> "EmailIterator":
    ...
    def filter_by_content_type(self, content_type: str) -> "EmailIterator":
        ...
        return self.filter(lambda email: any(att.content_type.startswith(content_type) for att in email.attachments))
    ...
    def sort_by_date(self, ascending: bool = True) -> "EmailIterator":
        ...
        sorted_emails = sorted(self._get_current_emails(), key=date_key, reverse=not ascending)
        return EmailIterator(sorted_emails)

    def sort_by_size(self, ascending: bool = True) -> "EmailIterator":
        """Sort emails by size."""
        sorted_emails = sorted(self._get_current_emails(), key=lambda email: email.size, reverse=not ascending)
        return EmailIterator(sorted_emails)

    def sort_by_subject(self, ascending: bool = True) -> "EmailIterator":
        """Sort emails by subject."""
        sorted_emails = sorted(self._get_current_emails(), key=lambda email: email.subject.lower(), reverse=not ascending)
        return EmailIterator(sorted_emails)
    ...
    def sort_by_sender(self, ascending: bool = True) -> "EmailIterator":
        ...
        sorted_emails = sorted(self._get_current_emails(), key=sender_key, reverse=not ascending)
        return EmailIterator(sorted_emails)

    def sort_by_attachment_count(self, ascending: bool = True) -> "EmailIterator":
        """Sort emails by number of attachments."""
        sorted_emails = sorted(self._get_current_emails(), key=lambda email: len(email.attachments), reverse=not ascending)
        return EmailIterator(sorted_emails)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_attachment_size': sum(email.total_attachment_size for email in emails),
    ...
    def save_all_to_directory(self, directory: Union[str, Path], filename_template: str = "{subject}_{date}") -> List[Path]:
    ...
    def deduplicate(self, key_func: Optional[Callable[[EmailMessage], str]] = None) -> "EmailIterator":
