...
def concurrent_pooling_example():
    ...
    def worker_task(worker_id: int) -> Dict[str, Any]:
        ...
        client = IMAPClient(
            host=HOST, username=USERNAME, password=PASSWORD, use_pool=True
        )
    ...
    avg_time_per_op = (
        sum(r["total_time"] for r in all_results) / total_operations
    ...
    print(
        f"  - Success rate: {((total_operations - total_errors) / total_operations * 100):.1f}%"
    )
...
def pool_configuration_example():
    ...
    clients = [IMAPClient.from_config(config, use_pool=True) for _ in range(3)]
...
def pool_performance_comparison():
    ...
    def performance_test(use_pool: bool, num_operations: int = 10) -> Dict[str, Any]:
        ...
        client = IMAPClient(
            host=HOST, username=USERNAME, password=PASSWORD, use_pool=use_pool
        )
    ...
    print(
        f"  - Avg connection time: {sum(no_pool_results['connection_times'])/len(no_pool_results['connection_times']):.3f}s"
    )
    print(
        f"  - Avg operation time: {sum(no_pool_results['operation_times'])/len(no_pool_results['operation_times']):.3f}s"
    )
    ...
    print(
        f"  - Avg connection time: {sum(pool_results['connection_times'])/len(pool_results['connection_times']):.3f}s"
    )
    print(
        f"  - Avg operation time: {sum(pool_results['operation_times'])/len(pool_results['operation_times']):.3f}s"
    )
    print(f"  - Errors: {pool_results['errors']}")

    # Calculate improvement
    time_improvement = (
        (no_pool_results["total_time"] - pool_results["total_time"])
        / no_pool_results["total_time"]
        * 100
    )
...
def pool_monitoring_example():
    ...
    def monitor_pool_stats():
        ...
        stats = get_pool_stats()
        print(
            f"  Pool Stats: {stats['active_pools']} pools, {stats['total_pooled_connections']} connections"
        )
