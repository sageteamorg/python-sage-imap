    ...
    try:
        ...
        for op_name, operation in operations:
            ...
            try:
                ...
                print(
                    f"  - Total operations: {snapshot['metrics']['total_operations']}"
                )
                print(
                    f"  - Average response time: {snapshot['metrics']['average_response_time']:.3f}s"
                )
        ...
        print(
            f"  - Success rate: {((final_metrics.total_operations - final_metrics.failed_operations) / final_metrics.total_operations * 100):.1f}%"
        )
...
def performance_monitoring_example():
    ...
    try:
        ...
        print(
            f"  - Client average response time: {final_metrics.average_response_time:.3f}s"
        )
...
def health_monitoring_example():
    ...
    def check_health_alerts(health_data: Dict[str, Any]) -> List[str]:
        ...
        if health_data.get("success_rate", 100) < 90:
            alerts.append(
                f"WARNING: Low success rate ({health_data['success_rate']:.1f}%)"
            )

        if health_data.get("average_response_time", 0) > 5.0:
            alerts.append(
                f"WARNING: High response time ({health_data['average_response_time']:.3f}s)"
            )
    ...
    try:
        ...
        for i in range(checks):
            ...
            health_history.append(
                {"timestamp": datetime.now(), "health": health.copy()}
            )
        ...
        if len(health_history) >= 2:
            ...
            print("Health trends:")
            print(
                f"  - Operations: {first_check['total_operations']} → {last_check['total_operations']}"
            )
            print(
                f"  - Success rate: {first_check['success_rate']:.1f}% → {last_check['success_rate']:.1f}%"
            )
            print(
                f"  - Response time: {first_check['average_response_time']:.3f}s → {last_check['average_response_time']:.3f}s"
            )
            ...
            time_trend = (
                last_check["average_response_time"]
                - first_check["average_response_time"]
            )
...
def metrics_export_example():
    ...
    try:
        ...
        print(
            f"imap_successful_connections_total {final_metrics.successful_connections}"
        )
        ...
        print(
            f"imap_average_response_time_seconds {final_metrics.average_response_time}"
        )
        print(
            f"imap_connection_uptime_seconds {final_metrics.connection_uptime.total_seconds()}"
        )
        ...
        if summary:
            ...
            if "performance" in summary:
                ...
                print(
                    f"Response time (avg/min/max): {perf['avg_response_time']:.3f}s / {perf['min_response_time']:.3f}s / {perf['max_response_time']:.3f}s"
                )
...
def real_time_monitoring_example():
    ...
    def display_dashboard(metrics: ConnectionMetrics, health: Dict[str, Any]):
        ...
        print(
            f"Connection Status: {status_icon} {'CONNECTED' if health['is_connected'] else 'DISCONNECTED'}"
        )
        print(f"Connection Age: {health['connection_age']:.1f}s")
        print()
