...
class IMAPClientManager:
    ...
    def connect(self) -> IMAPClient:
        ...
        self.client = IMAPClient.from_config(
            connection_config, use_pool=self.config.use_pool
        )
        ...
        logger.info(
            f"Connected to {self.config.host} in {self.config.environment} mode"
        )
        return self.client
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if len(self.metrics_history) >= 2:
            ...
            summary["trends"] = {
                "operations_growth": last["metrics"].total_operations
                - first["metrics"].total_operations,
                "success_rate_change": last["health"]["success_rate"]
                - first["health"]["success_rate"],
                "response_time_change": last["metrics"].average_response_time
                - first["metrics"].average_response_time,
            }
    ...
    for config in environments:
        ...
        try:
            with IMAPClientManager(config) as manager:
                ...
                print(
                    f"  - Response time: {current['metrics'].average_response_time:.3f}s"
                )
...
def custom_decorators_example():
    ...
    try:
        with IMAPClientManager(config) as manager:
            ...
            print("\nOperation Metrics:")
            print(
                f"  - Total operations: {summary['current']['metrics'].total_operations}"
            )
            print(
                f"  - Average response time: {summary['current']['metrics'].average_response_time:.3f}s"
            )
...
def connection_lifecycle_example():
    ...
    class LifecycleAwareClient:
        ...
        def on_operation(self, operation_name: str):
            ...
            logger.debug(
                f"Executing operation {self.operation_count}: {operation_name}"
            )
        ...
        def get_lifecycle_summary(self) -> Dict[str, Any]:
            ...
            return {
                ...
                "uptime": (
                    (datetime.now() - self.connection_events[0][1]).total_seconds()
...
def integration_patterns_example():
    ...
    class MetricsExporter:
        ...
        def export_to_json(
            self, metrics: ConnectionMetrics, health: Dict[str, Any]
        ) -> str:
        ...
        def send_to_monitoring_system(self, data: str, format_type: str):
            ...
            self.exported_metrics.append(
                {"timestamp": datetime.now(), "format": format_type, "data": data}
            )
    ...
    try:
        with IMAPClientManager(config) as manager:
            ...
            for event in event_summary["recent_events"]:
                print(
                    f"  - {event['type']}: {event.get('operation', 'N/A')} at {event['timestamp'].strftime('%H:%M:%S')}"
                )
...
def production_ready_patterns():
    ...
    class ProductionIMAPService:
        ...
        def execute_with_circuit_breaker(
            ...
            try:
                ...
                logger.error(
                    f"Circuit breaker failure {self.circuit_breaker_failures}/{self.circuit_breaker_threshold} for {operation_name}: {e}"
                )
                raise
    ...
    try:
        ...
        with service.manager:
            ...
            for op_name, operation in operations:
                try:
                    service.execute_with_circuit_breaker(operation, op_name)
            ...
            print(
                f"  - Circuit breaker failures: {health_status['circuit_breaker']['failures']}"
            )
            print(
                f"  - Circuit breaker open: {health_status['circuit_breaker']['is_open']}"
            )
            ...
            print(
                f"  - Monitoring enabled: {summary['configuration']['monitoring_enabled']}"
            )
...
def main():
    ...
    print("=" * 60)
    print(
        "This script demonstrates advanced features and integration patterns of IMAPClient."
    )
