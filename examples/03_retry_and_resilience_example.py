...
def custom_retry_decorator_example():
    ...
    def unreliable_operation(success_probability: float = 0.3) -> str:
        ...
        # Note: random.random() is used here only for demo purposes to simulate failures
        # In production code, use secrets.SystemRandom() for cryptographic purposes
        ...
        for op_name, operation in operations:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    execution_time = time.time() - start_time
                    print(
                        f"  ✗ Attempt {attempt + 1} failed: {e} (took {execution_time:.3f}s)"
                    )
...
def health_monitoring_example():
    ...
    try:
        ...
        for i in range(5):
            ...
            health = client.health_check()
            print(
                f"  Health check: Connected={health['is_connected']}, "
...
def connection_recovery_example():
    ...
    try:
        ...
        print(
            f"  - Success rate: {((metrics.total_operations - metrics.failed_operations) / metrics.total_operations * 100):.1f}%"
        )
...
def main():
    ...
    print("=" * 60)
    print(
        "This script demonstrates the retry logic and resilience features of IMAPClient."
    )
