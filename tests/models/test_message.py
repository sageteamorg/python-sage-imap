...
class TestMessageSet:
    ...
    def test_from_uids_success(self):
        ...
        assert msg_set.mailbox == "INBOX"
        assert (
            msg_set.msg_ids == "1001:1003"
        )  # Consecutive UIDs are optimized to ranges
    ...
    def test_from_sequence_numbers_success(self):
        ...
        with patch("sage_imap.models.message.logger") as mock_logger:
            ...
            assert (
                msg_set.msg_ids == "1:3"
            )  # Consecutive sequence numbers are optimized to ranges
    ...
    def test_intersection_different_types(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot intersect UID and sequence number"
        ):
            msg_set1.intersection(msg_set2)
...
class TestMessageSetEdgeCases:
    ...
    def test_message_set_initialization_error(self):
        ...
        with patch.object(
            MessageSet, "_validate_message_set", side_effect=ValueError("Test error")
        ):
    ...
    def test_message_set_first_last_id_mixed(self):
        ...
        assert (
            msg_set.get_last_id() >= 1
        )  # Last ID depends on actual parsing  # Max from range
