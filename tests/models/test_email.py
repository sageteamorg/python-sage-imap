...
from sage_imap.models.email import Attachment, EmailIterator, EmailMessage
...
class TestAttachment:
    ...
    def test_attachment_init_valid(self):
        ...
        attachment = Attachment(
            filename="test.txt", content_type="text/plain", payload=payload
        )
    ...
    def test_attachment_init_empty_filename(self):
        ...
        with pytest.raises(ValueError, match="Attachment filename cannot be empty"):
            Attachment(filename="", content_type="text/plain", payload=b"test")

    def test_attachment_init_empty_content_type(self):
        """Test attachment initialization with empty content type."""
        attachment = Attachment(filename="test.txt", content_type="", payload=b"test")
    ...
    def test_attachment_filename_sanitization(self):
        ...
        attachment = Attachment(
            filename="../../../etc/passwd", content_type="text/plain", payload=b"test"
        )
    ...
    def test_attachment_filename_length_limit(self):
        ...
        attachment = Attachment(
            filename=long_filename, content_type="text/plain", payload=b"test"
        )
    ...
    def test_attachment_is_image_property(self):
        ...
        image_attachment = Attachment(
            filename="test.jpg", content_type="image/jpeg", payload=b"test"
        )

        text_attachment = Attachment(
            filename="test.txt", content_type="text/plain", payload=b"test"
        )
    ...
    def test_attachment_save_to_file_logging(self, mock_logger):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                f"Attachment saved to: {saved_path}" in mock_logger.info.call_args[0][0]
            )
    ...
    def test_read_from_eml_file_wrong_extension(self):
        ...
        try:
            with patch("sage_imap.models.email.logger") as mock_logger:
                ...
                assert (
                    "doesn't have .eml extension" in mock_logger.warning.call_args[0][0]
                )
        finally:
            os.unlink(temp_file_path)
...
"""
    ...
    def test_safe_header_decode_error(self):
        ...
        with patch("sage_imap.models.email.logger") as mock_logger:
            with patch(
                "email.header.decode_header", side_effect=Exception("Decode error")
            ):
    ...
    def test_parse_email_addresses_multiple(self):
        ...
        assert len(result) == 2
        assert all(
            isinstance(addr, str) for addr in result
        )  # EmailAddress is a NewType based on str
    ...
    def test_extract_body_multipart(self):
        ...
        with patch.object(
            email, "decode_payload", side_effect=["Plain text", "HTML text"]
        ):
            plain_body, html_body = email.extract_body(mock_message)
    ...
    def test_is_multipart_property(self):
        ...
        email_with_both_bodies = EmailMessage(
            message_id="test", plain_body="plain", html_body="<p>html</p>"
        )

        simple_email = EmailMessage(message_id="test", plain_body="plain")
    ...
    def test_has_html_body_method(self):
        ...
        email_with_html = EmailMessage(
            message_id="test", html_body="<p>HTML content</p>"
        )
    ...
    def test_has_plain_body_method(self):
        ...
        email_with_plain = EmailMessage(message_id="test", plain_body="Plain content")
    ...
    def test_get_body_preview_method(self):
        ...
        email = EmailMessage(message_id="test", plain_body="A" * 150)
    ...
    def test_get_body_preview_short_body(self):
        ...
        email = EmailMessage(message_id="test", plain_body="Short body")
    ...
    def test_get_body_preview_html_fallback(self):
        ...
        email = EmailMessage(message_id="test", html_body="<p>HTML content</p>")
    ...
    def test_is_reply_method(self):
        ...
        reply_email = EmailMessage(message_id="test", subject="Re: Original subject")

        normal_email = EmailMessage(message_id="test", subject="Normal subject")
    ...
    def test_is_forward_method(self):
        ...
        forward_email = EmailMessage(message_id="test", subject="Fwd: Original subject")
...
class TestEmailIterator:
    ...
    def test_email_iterator_filter_by_recipient(self):
        ...
        # Exact match
        filtered = iterator.filter_by_recipient(
            "recipient1@example.com", exact_match=True
        )
        assert len(filtered) == 2
